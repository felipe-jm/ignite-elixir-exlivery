defmodule Exlivery.Orders.ItemTest do
  ...
  describe "build/4" do
    test "when all params are valid, returns an item" do
      response = Item.build("Pizza de queijo", :pizza, "35.5", 1)
    ...
    test "when there is an invalid category, returns an error" do
      response = Item.build("Pizza de queijo", :invalid_category, "35.5", 1)
    ...
    test "when there is an invalid price, returns an error" do
      response = Item.build("Pizza de queijo", :pizza, "invalid_price", 1)
    ...
    test "when there is an invalid quantity, returns an error" do
      response = Item.build("Pizza de queijo", :pizza, "35.5", 0)
